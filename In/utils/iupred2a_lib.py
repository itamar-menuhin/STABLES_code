...
os.chdir('/mnt/c/Users/itamar/Desktop/pythonProject_model_improvement')
...
def iupred(seq, mode='long'):
    ...
    if mode == "short":
        ...
        uc = 25
        wc = 10
        mtx = read_matrix(os.path.join(PATH, 'data', 'iupred2_short_energy_matrix'))
        histo, histo_min, histo_max, histo_step = read_histo(os.path.join(PATH, 'data', 'short_histogram'))

    elif mode == 'glob':
        lc = 1
        uc = 100
        wc = 15
        mtx = read_matrix(os.path.join(PATH, 'data', 'iupred2_long_energy_matrix'))
        histo, histo_min, histo_max, histo_step = read_histo(os.path.join(PATH, 'data', 'long_histogram'))
...
def anchor2(seq):
    ...
    mtx = read_matrix(os.path.join(PATH, 'data', 'anchor2_energy_matrix'))

    interface_comp = {}
    with open(os.path.join(PATH, 'data', 'anchor2_interface_comp')) as _fn:
        for line in _fn:
